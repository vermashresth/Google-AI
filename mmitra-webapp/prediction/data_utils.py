...
def date_to_int(date: str, fmt: str = "%Y-%m-%d") -> int:
    ...
    try:
        ...
        logging.warning("Can't convert %s using format %s: %s. Outputting NA.", date, fmt, str(e))
        return pd.NaT
...
def phowner_to_int(inp: str) -> int:
    ...
    if not inp or inp.lower().strip() not in vals.keys():
...
def gest_age_to_list(gest_age: str) -> pd.Series:
    ...
    try:
        ...
        if gest_age == "intro" or gest_age == "introivr":
            return [0, 0, 0]
        data = gest_age.split(".")
