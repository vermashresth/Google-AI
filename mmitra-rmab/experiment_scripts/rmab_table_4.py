...
def run_experiment(train_beneficiaries, train_transitions, test_beneficiaries, call_data, CONFIG, features_dataset, beneficiary_data):
    ...
    group_transition_probabilities = get_group_transition_probabilities(train_beneficiaries, train_transitions, beneficiary_data)
    cols = [
        "P(L, I, L)", "P(L, I, H)", "P(H, I, L)", "P(H, I, H)", "P(L, N, L)", "P(L, N, H)", "P(H, N, L)", "P(H, N, H)",
    ]
    ...
    # whittle_indices = {'user_id': [], 'whittle_index': [], 'cluster': [], 'start_state': [], 'lstm_prediction': []}
    for i in test_beneficiaries.index:
        user_id = test_beneficiaries.loc[i, "user_id"]
        beneficiary_features = beneficiary_data.loc[beneficiary_data['user_id']==user_id, :]

        pilot_date_num = (pd.to_datetime("2020-10-23", format="%Y-%m-%d") - pd.to_datetime("2018-01-01", format="%Y-%m-%d")).days

        if CONFIG['transitions'] == 'weekly':
            past_days_calls = call_data[
                (call_data["user_id"]==user_id)&
                (call_data["startdate"]<pilot_date_num)&
                (call_data["startdate"]>=pilot_date_num - 7)
            ]
        else:
            past_days_calls = call_data[
                (call_data["user_id"]==user_id)&
                (call_data["startdate"]<pilot_date_num)&
                (call_data["startdate"]>=pilot_date_num - 30)
            ]
        ...
        test_beneficiaries.loc[i, "Whittle Index"] = m_values[curr_cluster, curr_state]
...
def run_third_pilot(all_beneficiaries, transitions, call_data, CONFIG, features_dataset, pilot_data, beneficiary_data):
    ...
    # cluster_transition_probabilities.to_csv('outputs/non_linear_clusters/{}_{}_transition_probabilities_{}.csv'.format(CONFIG['transitions'], CONFIG['clustering'], CONFIG['clusters']))
    q_values, m_values = plan(cluster_transition_probabilities, CONFIG)
    ...
    whittle_indices = {'user_id': [], 'whittle_index': [], 'cluster': [], 'RCT-group': [], 'start_state': [], 'lstm_prediction': [], 'Post-Intervention-E2C': []}
    for idx, puser_id in enumerate(pilot_user_ids):
        if puser_id not in all_beneficiaries['user_id'].to_list():
            continue
        pilot_pd_features = beneficiary_data.loc[beneficiary_data['user_id']==puser_id, :]
        pilot_date_num = (pd.to_datetime("2020-10-23", format="%Y-%m-%d") - pd.to_datetime("2018-01-01", format="%Y-%m-%d")).days
        ...
        gold_e2c = all_beneficiaries[all_beneficiaries['user_id'] == puser_id]['Post-intervention Day: E2C Ratio'].item()
        whittle_indices['Post-Intervention-E2C'].append(gold_e2c)
        rct_group = all_beneficiaries[all_beneficiaries['user_id'] == puser_id]['Group'].item()
        whittle_indices['RCT-group'].append(rct_group)
    # ipdb.set_trace()
    df = pd.DataFrame(whittle_indices)
    df = df.sort_values('whittle_index', ascending=False)
    df.to_csv('outputs/cluster_non_intervention/{}_{}_may_stats_{}.csv'.format(CONFIG['transitions'], CONFIG['clustering'], CONFIG['clusters']))

    # ipdb.set_trace()

    return

def run_and_repeat(beneficiaries, transitions, call_data, CONFIG, features_dataset, beneficiary_data):
    ...
    for train_beneficiaries, test_beneficiaries in beneficiaries:
        ...
        results, notes = run_experiment(train_beneficiaries, train_transitions, test_beneficiaries, call_data, CONFIG, features_dataset, beneficiary_data)
...
run_third_pilot(all_beneficiaries, transitions, call_data, CONFIG, features_dataset, 'may_data', beneficiary_data)

# results, avgs = run_and_repeat(beneficiary_splits, transitions, call_data, CONFIG, features_dataset, beneficiary_data)
# print("-"*60)
# print(results)
# print(avgs)
# print("-"*60)