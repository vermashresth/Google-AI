...
def run_third_pilot(all_beneficiaries, transitions, call_data, CONFIG, features_dataset, pilot_data, beneficiary_data):
    ...
    cluster_transition_probabilities, cls = get_static_feature_clusters(all_beneficiaries, transitions, features_dataset, CONFIG['clusters'])
    cols = [
        "P(L, I, L)", "P(L, I, H)", "P(H, I, L)", "P(H, I, H)", "P(L, N, L)", "P(L, N, H)", "P(H, N, L)", "P(H, N, H)", 
    ]
    # ipdb.set_trace()
    ...
    # cluster_transition_probabilities.to_csv('outputs/feature_clusters/{}_{}_transition_probabilities_{}.csv'.format(CONFIG['transitions'], CONFIG['clustering'], CONFIG['clusters']))
    q_values, m_values = plan(cluster_transition_probabilities, CONFIG)
