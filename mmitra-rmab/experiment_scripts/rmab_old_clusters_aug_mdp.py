...
def kmeans_missing(X, n_clusters, max_iter=10):
    ...
    return labels, centroids, X_hat, cls
...
def run_third_pilot(all_beneficiaries, transitions, call_data, CONFIG, features_dataset, pilot_data, beneficiary_data):
    ...
    labels, centroids, X_hat, cls = kmeans_missing(group_transition_probabilities[cols], CONFIG['clusters'], max_iter=1000)
    ...
    ipdb.set_trace()
    # cluster_transition_probabilities['P(L, I, L)'] = cluster_transition_probabilities.fillna(cluster_transition_probabilities.mean())
    # cluster_transition_probabilities.to_csv('outputs/old_clusters/all_{}_cluster_transition_probabilities_{}.csv'.format(CONFIG['transitions'], CONFIG['clusters']))
    q_values, m_values = plan(cluster_transition_probabilities, CONFIG)
    ...
    whittle_indices = {'user_id': [], 'whittle_index': [], 'cluster': []}
    ...
    # df.to_csv('outputs/old_clusters/all_{}_pilot_stats_{}.csv'.format(CONFIG['transitions'], CONFIG['clusters']))
    import ipdb
    ipdb.set_trace()
