...
def run_third_pilot(all_beneficiaries, transitions, call_data, CONFIG, features_dataset, beneficiary_data):
    ...
    return total_mse, total_count, avg_mse, std, count2

df_dict = {"# of clusters":[],"mse":[],"count":[],"avg_mse":[],"std":[],"count2":[]}
for k in [10, 20, 30, 40, 50, 75, 100, 200, 300, 400, 500]:
  CONFIG["clusters"] = k
  mse, count, avg_mse, std, count2 = run_third_pilot(all_beneficiaries, transitions, call_data, CONFIG, features_dataset, beneficiary_data)
