...
intervention_dict = {}
for file in ['250_week1_290421', '400_week2_060521', '400_week3_120521', '400_week4_180521', '435_week5_240521', '600_week6_310521']:
...
# ipdb.set_trace()
intervention_call_status = {}
for file in ['week1', 'week2', 'week3', 'week4']:
    df = pd.read_excel('neha_lists/neha_calling_status_{}.xlsx'.format(file))
    user_list = df['UserID'].to_list()
    for user in user_list:
        val = df[df['UserID'] == user]['Call Outcome'].item()
        if user not in intervention_call_status:
            intervention_call_status[user] = [str(val)]
        else:
            intervention_call_status[user].append(str(val))

out_dict = {'user_id': [], 'cluster': [], 'arm': [], 'registration_date': [], 'intervention_week': [], 'call_status': [], 'week1_whittle': [], 'after_intervention_E/C' : []}
...
for user_id in tqdm(all_user_ids):
    if user_id not in intervention_dict or len(intervention_dict[user_id]) == 1:
        continue
    if arm_dict[user_id] != 'rmab':
        continue
    ...
    out_dict['intervention_week'].append((', ').join(intervention_dict[user_id]))
    # else:
        # out_dict['intervention_week'].append('')
    if user_id in intervention_call_status:
        out_dict['call_status'].append((', ').join(intervention_call_status[user_id]))
    else:
        out_dict['call_status'].append('')

    curr_state = None
    countc, counte = 0, 0
    for i, date_val in enumerate(['2021-04-26', '2021-05-03', '2021-05-10', '2021-05-17', '2021-05-24', '2021-05-31', '2021-06-07']):
...
df.to_csv('repeat_call_analysis_week_{}.csv'.format(CONFIG['current_week']))

exit()

df = pd.read_csv('outputs/analysis_lists/all_analysis_week_7.csv')
...
for arm in ['rmab', 'round_robin', 'control']:
