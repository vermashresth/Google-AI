...
def get_static_feature_clusters(train_beneficiaries, train_transitions, features_dataset, n_clusters):
    ...
    for i in range(n_clusters):
        ...
        probs, _ = get_transition_probabilities(cluster_b_user_ids, train_transitions, min_support=7)
        probs['cluster'] = i
    ...
    return cluster_transition_probabilities, cls, train_beneficiaries
...
def run_third_pilot(all_beneficiaries, transitions, call_data, CONFIG, features_dataset, pilot_data, beneficiary_data):
    ...
    cluster_transition_probabilities, cls, train_beneficiaries = get_static_feature_clusters(all_beneficiaries, transitions, features_dataset, CONFIG['clusters'])
    cols = [
        "P(L, I, L)", "P(L, I, H)", "P(H, I, L)", "P(H, I, H)", "P(L, N, L)", "P(L, N, H)", "P(H, N, L)", "P(H, N, H)", 
    ]
    # ipdb.set_trace()
    ...
    cluster_transition_probabilities.to_csv('outputs/feature_clusters/{}2_{}_transition_probabilities_{}.csv'.format(CONFIG['transitions'], CONFIG['clustering'], CONFIG['clusters']))
    exit()
    q_values, m_values = plan(cluster_transition_probabilities, CONFIG)
    ...
    df.to_csv('outputs/feature_clusters/{}_{}_pilot_stats_{}.csv'.format(CONFIG['transitions'], CONFIG['clustering'], CONFIG['clusters']))
