...
def plan(transition_probabilities, CONFIG):
    ...
    v_values = np.zeros((CONFIG['clusters'], CONFIG['problem']['horizon'], len(CONFIG['problem']['states'])))
    q_values = np.zeros((CONFIG['clusters'], CONFIG['problem']['horizon'], len(CONFIG['problem']['states']), len(CONFIG['problem']['actions'])))
    high_m_values = 1 * np.ones((CONFIG['clusters'], CONFIG['problem']['horizon'], len(CONFIG['problem']['states'])))
    low_m_values = -1 * np.ones((CONFIG['clusters'], CONFIG['problem']['horizon'], len(CONFIG['problem']['states'])))
    # m_values = np.zeros((CONFIG['clusters'], CONFIG['problem']['horizon'], len(CONFIG['problem']['states'])))
    ...
    for cluster in range(CONFIG['clusters']):
        ...
        v_values[cluster, :, :] = np.zeros((CONFIG['problem']['horizon'], len(CONFIG['problem']['states'])))
        q_values[cluster, :, :, :] = np.zeros((CONFIG['problem']['horizon'], len(CONFIG['problem']['states']), len(CONFIG['problem']['actions'])))

        nT = CONFIG['problem']['horizon']
        nS = len(CONFIG['problem']['states'])
        nA = len(CONFIG['problem']['actions'])
        for curr_h in range(nT - 1, -1, -1):
            max_q_diff = np.inf
            ...
            while max_q_diff > 1e-5:
                ...
                if type(prev_m_values) != type(None) and abs(prev_m_values - m_values).max() < 1e-20:
                    break
                max_q_diff = 0
                v_values[cluster, curr_h, :] = np.zeros((len(CONFIG['problem']['states'])))
                q_values[cluster, curr_h, :, :] = np.zeros((len(CONFIG['problem']['states']), len(CONFIG['problem']['actions'])))
                
                for hh in range(nT - 1, curr_h - 1, -1):
                    for ss in range(nS):
                        max_q = -np.inf
                        for aa in range(nA):
                            q_aa = get_reward(CONFIG['problem']['states'][ss], CONFIG['problem']['actions'][aa], m_values[cluster, curr_h, ss])
                            if hh < nT - 1:
                                for ns in range(nS):
                                    q_aa += CONFIG['gamma'] * t_probs[ss, ns, aa] * v_values[cluster, hh + 1, ns]
                            if q_aa > max_q:
                                max_q = q_aa
                            q_values[cluster, hh, ss, aa] = q_aa
                        # m_values[cluster, hh, ss] = q_values[cluster, hh, ss, 1] - q_values[cluster, hh, ss, 0]
                        v_values[cluster, hh, ss] = max_q

                state_idx = -1
                for curr_state in range(nS):
                    if abs(q_values[cluster, curr_h, curr_state, 0] - q_values[cluster, curr_h, curr_state, 1]) > max_q_diff:
                        state_idx = curr_state
                        max_q_diff = abs(q_values[cluster, curr_h, curr_state, 0] - q_values[cluster, curr_h, curr_state, 1])

                if max_q_diff > 1e-5 and q_values[cluster, curr_h, state_idx, 0] < q_values[cluster, curr_h, state_idx, 1]:
                    low_m_values[cluster, curr_h, state_idx] = m_values[cluster, curr_h, state_idx]
                elif max_q_diff > 1e-5 and q_values[cluster, curr_h, state_idx, 0] > q_values[cluster, curr_h, state_idx, 1]:
                    high_m_values[cluster, curr_h, state_idx] = m_values[cluster, curr_h, state_idx]
        # ipdb.set_trace()
...
def run_third_pilot(all_beneficiaries, transitions, call_data, CONFIG, features_dataset, pilot_data, beneficiary_data):
    ...
    for idx, puser_id in enumerate(pilot_user_ids):
        ...
        whittle_indices['whittle_index'].append(m_values[curr_cluster, 0, curr_state])
    ...
    df.to_csv('outputs/finite_horizon/{}_{}_pilot_stats_{}.csv'.format(CONFIG['transitions'], CONFIG['clustering'], CONFIG['clusters']))
