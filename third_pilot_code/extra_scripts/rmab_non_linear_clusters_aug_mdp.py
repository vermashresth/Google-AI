...
# get_all_transition_probabilities(all_beneficiaries, transitions)
...
def run_third_pilot(all_beneficiaries, transitions, call_data, CONFIG, features_dataset, pilot_data, beneficiary_data):
    ...
    for i in range(centroids.shape[0]):
        ...
        # print('Num List: {}'.format(num_lst))
        t_probs["cluster"] = i
        t_probs['count'] = curr_group_transition_probabilities['count'].sum()
        # for j in range(len(cols)):
        #   t_probs[cols[j]] = centroids[i, j]
        num_lst['cluster'] = i
        num_lst['count'] = curr_group_transition_probabilities['count'].sum()

        cluster_transition_probabilities = cluster_transition_probabilities.append(t_probs, ignore_index=True)
        cluster_freq = cluster_freq.append(num_lst, ignore_index=True)
    
    # cluster_freq.to_csv('frequencies.csv')
    # cluster_transition_probabilities.to_csv('missing_probs.csv')
    ...
    train_b_ids = all_beneficiaries['user_id'].to_list()
    ...
    for user_id in train_b_ids:
        flag = False
        pilot_pd_features = beneficiary_data.loc[beneficiary_data['user_id']==user_id, :]
        ...
        user_cluster = group_transition_probabilities.loc[
    ...
    # cluster_transition_probabilities.to_csv('outputs/cluster_non_intervention/{}2_{}_transition_probabilities_{}.csv'.format(CONFIG['transitions'], CONFIG['clustering'], CONFIG['clusters']))
    exit()
    q_values, m_values = plan(cluster_transition_probabilities, CONFIG)
    ...
    for idx, puser_id in enumerate(pilot_user_ids):
        ...
        pilot_date_num = (pd.to_datetime("2021-04-01", format="%Y-%m-%d") - pd.to_datetime("2018-01-01", format="%Y-%m-%d")).days
    ...
    # df.to_csv('outputs/non_linear_clusters/{}_{}_pilot_stats_{}.csv'.format(CONFIG['transitions'], CONFIG['clustering'], CONFIG['clusters']))
