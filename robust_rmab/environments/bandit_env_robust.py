...
class ARMMANRobustEnv(gym.Env):
    def __init__(self, N, B, seed, data=None):#, REWARD_BOUND):
        ...
        def compute_interval_stats(param_ranges):
        ...
        # compute_interval_stats(param_ranges)

        self.PARAMETER_RANGES = feasible_param_ranges

        # Set these parameters from global loaded pickled file
        # TODO: make this more clean
        orig_cluster_mapping, self.max_cluster_size = info_dict['cluster_mapping'], info_dict['max_cluster_size']
        self.cluster_mapping = [int(indiv) for indiv in orig_cluster_mapping]

        # count and display custer sizes
        self.cluster_size = {}
        for cluster in range(self.n_clusters):
            self.cluster_size[cluster] = 0
        for indiv, cluster in enumerate(self.cluster_mapping):
            self.cluster_size[cluster] += 1

        print('---------------------------')
        print('cluster sizes')
        for cluster in range(self.n_clusters):
            print(f'cluster {cluster}: {self.cluster_size[cluster]}')
