...
def get_offline_dataset(policy, T):
    ...
    for idx in range(n_instance):
        ...
        ope_simulator_subset = opeSimulator(offline_traj[:, :, :, :, all_idx], n_benefs, L, n_states, OPE_sim_n_trials, gamma, beh_policy_name='random', R_data=R_data_ope_sim, env=env, H=H, use_informed_prior=True)
